...
object FunSets {
  ...
  def exists1(s: Set, p: Int => Boolean): Boolean = {
    def iterate(a: Int): Boolean =
      if (a > bound) false // se ja nao teve um bound antes que retornou true, claro que eh falso
      else if (contains(s, a) && p(a)) true
      else iterate(a + 1)
    iterate(-bound)
  }
  // one liner
  // !p(x) funciona como a condicao de parada from the evaluation if(contains(s,a)) p(a)
